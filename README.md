...
- each initial class matrix has shape **784 × 200**
- each updated class matrix has shape **784 × 220**
...
For each letter class, the training samples form a matrix $A_i$. A full QR factorization is computed as
...
When a new training vector $x$ is appended as a column, the existing factors are updated rather than recomputed from scratch.
...
The earlier version of this project mixed this `givens()` convention with the opposite rotation application. The current implementation reconciles the convention with the source algorithm and validates the result numerically before applying it to EMNIST.

The one-column update is implemented as
...
The tests verify four properties:
...
3. **Upper-trapezoidal structure**

   Entries below the diagonal of $R_{\text{new}}$ should remain approximately zero.

4. **Reference subspace agreement**

   The updated column space is compared with the one obtained from `numpy.linalg.qr`.

### Synthetic tests

The synthetic tests passed for all tested matrix sizes. Errors were approximately at machine precision:
...
For class A, the augmented matrix had full column rank: **220 / 220**.

The NumPy reduced-QR reconstruction error was **1.008e-15**, and the column-space projector difference between the incremental update and NumPy QR was **7.993e-15**.
...
Using 200 training samples per class, the overall accuracy was **70.58%**.

![Baseline per-class accuracy](assets/baseline-per-class-accuracy.png)

### After incremental QR updates

After appending 20 additional training samples per class, the overall accuracy was **68.46%**, a change of **−2.12 percentage points**.

The experiment therefore produced lower classification accuracy after adding the additional samples.
...
This distinction matters: the Hammarling–Lucas update algorithms reduce arithmetic relative to recomputing a factorization, but a Python implementation containing many small rotations should not automatically be expected to outperform optimized compiled LAPACK/BLAS routines.

## Implementation Scope and Limitations

- A full **784 × 784** matrix $Q$ is stored for each of the 26 classes.
...
Official report: [Manchester Institute for Mathematical Sciences](http://eprints.maths.manchester.ac.uk/1192/)
